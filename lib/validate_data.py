...
def log_invalid_row(table_name, row):
    ...
    db_connection.execute(f"CREATE TABLE IF NOT EXISTS invalid_data (id SERIAL PRIMARY KEY, intended_table VARCHAR(100), row_contents VARCHAR(100), reason VARCHAR(100), missing_fields VARCHAR(100))")
    ...
    if not result:
        ...
        db_connection.execute(query, (table_name, str(row), "Data missing from row", [item for item in row.keys() if row[item] == '']))
